...
public:
        ...
        calibration_manager(new camera_info_manager::CameraInfoManager(node_handle,camera_name))
    {
        ...
        node_handle.param<std::string>("frame_id", frame_id, "photonfocus_camera_link");

        std::cout << std::setw(80) << std::setfill(' ') << std::left << "===== PhotonFocus Camera STARTED ===== " << std::endl;
    }
    ...
    ~PhotonFocusDriver()
    {
        camera->stop();
        camera.reset();
        std::cout << "===== PhotonFocus Camera STOPPED  ===== " << std::endl;
    }
    ...
    void configCb(photonfocus_camera::photonfocus_monoConfig & config, uint32_t level)
    {
        ...
        camera->setDeviceAttribute<PvGenFloat,double>("ExposureTime",config.ExposureTime);
        camera->setDeviceAttribute<PvGenBoolean,bool>("AcquisitionFrameRateEnable",config.AcquisitionFrameRateEnable);
        if(config.AcquisitionFrameRateEnable)
            camera->setDeviceAttribute<PvGenFloat,double>("AcquisitionFrameRate",config.AcquisitionFrameRate);
